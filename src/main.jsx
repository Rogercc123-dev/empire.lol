...
function App(){
  ...
  return <div className="app-shell">
          ...
          <div className="stage-inner" style={{transform:`translate(${pan.x}px,${pan.y}px) scale(${zoom})`}}>
            ...
            {!currentWorld ? mergedWorlds.map((world,index)=>{
              ...
              return <button key={world.id} className={`world-orb ${toneClass(world.tone)}`} style={{left:`${p.x}%`,top:`${p.y}%`,width:p.size,height:p.size,transform:`translate(-50%,-50%) rotate(${p.rot}deg)`}} onClick={()=>openWorld(world.id)} onMouseEnter={()=>setHovered(world.id)} onMouseLeave={()=>setHovered(null)}>
                ...
                <span className="orbit-topic orbit-one">{world.topics[0].name}</span><span className="orbit-topic orbit-two">{world.topics[Math.min(4,world.topics.length-1)].name}</span>
              </button>
            }) : <>
    ...
    {activeTopic&&(()=>{const topic=currentWorld?.topics.find(t=>t.name===activeTopic);if(!topic)return null;return <div className="modal-backdrop" onClick={()=>setActiveTopic(null)}><div className="territory-modal" onClick={e=>e.stopPropagation()}><button className="close-btn" onClick={()=>setActiveTopic(null)}><X size={20}/></button><div className="modal-topline"><span className="modal-status">{currentWorld.name.toUpperCase()} / TERRITORY</span><span className="kicker">LIVE</span></div><h3>{topic.name}</h3><div className="modal-stats"><div><span>Current owner</span><b>{topic.owner||'Unclaimed'}</b></div><div><span>Current stake</span><b>£{topic.stake}</b></div></div><div className="related-list"><span>Connected</span><div>{(LINKS[topic.name]||[]).slice(0,6).map(n=><button key={n} onClick={()=>setActiveTopic(n)}>{n}</button>)}</div></div><button className="primary-btn wide" onClick={()=>claim(topic)}>{topic.owner?'Conquer territory':'Claim territory'} <ArrowUpRight size={17}/></button><p className="demo-note"><Sparkles size={14}/> Prototype mode — checkout is mocked.</p></div></div>})()}
  </div>
}
